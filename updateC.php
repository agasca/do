...
*******/
	...
	if(!isset($_SESSION['name'])){
		...
		//pagination
		$per_page = 6;	
		$pages_query = mysql_query("select count('id') from cuentas");
		$pages = ceil(mysql_result($pages_query, 0) / $per_page);
		$page = (isset($_GET['page'])) ? (int)$_GET['page'] : 1;
		$start = ($page - 1) * $per_page;
		$query = mysql_query("select * from cuentas  LIMIT $start, $per_page");
		//pagination
		...
		$smarty->display('updateC.tpl');				// desplegamos la plantilla indicando su nombre
		...
		echo "<center>";
		if(!($page<=1)){
			echo "<a href='updateC.php?page=$prev'>Previo</a> ";	
		}
		if($pages>=1){
			for($x=1;$x<=$pages;$x++){
		...
		if(!($page>=$pages)){
			echo "<a href='updateC.php?page=$next'>Siguiente</a> ";
		}	
		echo "</center>";
		//pagination
